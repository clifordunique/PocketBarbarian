...
public class GuiSimpleCharacterBackgroundController : MonoBehaviour {
    ...
    private void Init() {
        ...
        bgrTopLeftGo = Utils.InstantiateSpriteGameObject(bgrTopLeft, Constants.SORTING_LAYER_DIALOGUE, 1, transform);
        bgrTopRightGo = Utils.InstantiateSpriteGameObject(bgrTopRight, Constants.SORTING_LAYER_DIALOGUE, 1, transform);

        bgrBottomCenterGo = Utils.InstantiateSpriteGameObject(bgrBottomCenter, Constants.SORTING_LAYER_DIALOGUE, 0, transform);
        bgrBottomLeftGo = Utils.InstantiateSpriteGameObject(bgrBottomLeft, Constants.SORTING_LAYER_DIALOGUE, 1, transform);
        bgrBottomRightGo = Utils.InstantiateSpriteGameObject(bgrBottomRight, Constants.SORTING_LAYER_DIALOGUE, 1, transform);
    ...
    public Vector2 GetSize() {
        ...
        if (showArrow) {
            ...
            size = new Vector2(srCenter.bounds.size.x + (2 * srLeft.bounds.size.x), srCenter.bounds.size.y + srTopCenter.bounds.size.y + srArrow.bounds.size.y);
        } else {
            size = new Vector2(srCenter.bounds.size.x + (2 * srLeft.bounds.size.x), srCenter.bounds.size.y + srTopCenter.bounds.size.y);
        }
